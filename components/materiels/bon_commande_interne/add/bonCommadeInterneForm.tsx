...
import {
  ...
  editBonCommandeInterne,
  cancelEdit,
  updateBonCommandeInterne,
} from "../../../../redux/features/bon_commande_interne/bonCommandeInterneSlice";
import {
  createArticleCommandeInterne,
  deleteArticleCommandeInterne,
  updateArticleCommande,
} from "../../../../redux/features/bon_commande_interne/articleCommandeSlice";
...
export default function BonCommandeInterneForm() {
  ...
  const { isEditing, bonCommandeInterne } = useAppSelector(
    ...
    try {
      const updateData = {
        ...
        montantTotal: valuesArticle.reduce(
          (acc: any, curr: any) => acc + curr.valueArticle,
          0
        ),
      };
      ...
      if (isEditing) {
        const response = await dispatch(
          updateBonCommandeInterne({ id, updateData })
        );
      ...
      <Container maxWidth="xl" sx={{ paddingBottom: 8 }}>
        <Formik
          ...
          initialValues={{
            ...
            dateBonCommande:
              isEditing && bonCommandeInterne?.dateBonCommande
                ? new Date(bonCommandeInterne?.dateBonCommande)
                : new Date(),

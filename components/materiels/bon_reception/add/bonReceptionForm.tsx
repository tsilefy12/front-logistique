...
import {
  ...
  getBonReception,
  updateBonReception,
} from "../../../../redux/features/bon_reception/bonReceptionSlice";
...
export default function BonReceptionForm() {
  ...
  const { isEditing, bonReception } = useAppSelector(
    (state) => state.bonReceptions
  );
    ...
    try {
      if (isEditing) {
        ...
        const response = await dispatch(
          updateBonReception({ id, updateDataBR })
        );
    ...
    <>
      <Container maxWidth="xl" sx={{ paddingBottom: 8 }}>
        <Formik
          ...
          initialValues={{
            bce:
              isEditing && bonReception.bce
                ? bonReception.bce
                : isEditing && bonReception.bci
                ? bonReception.bci
                : "",
            reference: isEditing ? bonReception.reference : "",
            dateReception:
              isEditing && bonReception.dateReception
                ? new Date(bonReception.dateReception)
                : new Date(),
            type: "",
            etat: isEditing ? bonReception.etat : "",
            observation: isEditing ? bonReception.observation : "",
            designation: "",
            quantite: 0,
          }}

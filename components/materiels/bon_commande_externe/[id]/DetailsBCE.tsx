...
const DetailsBCE = () => {
  ...
  const { bonCommandeExterne } = useAppSelector(
  ...
  useEffect(() => {
    const data = {
      ...
      demandeur: total.find((e: any) => e.id === bonCommandeExterne?.demandeur)
        ?.name,
      grant: grantList.find((e: any) => e.id === bonCommandeExterne?.grant)
        ?.code,
      ligneBudgetaire: budgetLineList.find(
        (e: any) => e.id === bonCommandeExterne?.ligneBudgetaire
      )?.code,
                ...
                <Grid item xs={12} md={12}>
                  <InfoItems direction="row" spacing={2}>
                    ...
                    <Typography variant="body1" color="gray">
                      {
                        total.find(
                          (e: any) => e.id === bonCommandeExterne?.demandeur
                        )?.name
                      }
                    </Typography>
                  </InfoItems>
                ...
                <Grid item xs={12} md={12}>
                  <InfoItems direction="row" spacing={2}>
                    ...
                    <Typography variant="body1" color="gray">
                      {
                        grantList.find(
                          (e: any) => e.id === bonCommandeExterne?.grant
                        )?.code
                      }
                    </Typography>
                  </InfoItems>
                    ...
                    <Typography variant="body1" color="gray">
                      {
                        budgetLineList.find(
                          (e: any) =>
                            e.id === bonCommandeExterne?.ligneBudgetaire
                        )?.code
                      }
                    </Typography>
                  </InfoItems>
          ...
          <FormContainer spacing={2}>
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 700 }} aria-label="simple table">
                ...
                <TableBody>
                  {bonCommandeExterne.articleCommandeBce?.map(
                    (item: any, index: any) => (
                      <TableRow
                        key={index}
                        sx={{

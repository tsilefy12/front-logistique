...
import {
  BonCommandeExternInitialState,
  BonCommandeExterneItem,
  ...
  async (data: BonCommandeExterneItem, thunkAPI) => {
    try {
      const response = await axios.post(
        "/logistique/bon-de-commande-externe",
        data
      );
      thunkAPI.dispatch(
        enqueueSnackbar({
...
export const getBonCommandeExternes = createAsyncThunk(
  ...
  async (data: { args?: any }, thunkAPI) => {
    try {
      ...
      const response = await axios.get("/logistique/bon-de-commande-externe", {
        params,
      });
      return response.data;
    } catch (error: any) {
...
export const updateBonCommandeExterne = createAsyncThunk(
  "fourniture_consommable/updateBonCommandeExterne",
  async (
    data: { id: string; bonCommandeExterne: BonCommandeExterneItem },
    thunkAPI
  ) => {
    try {
      const response = await axios.patch(
        `/logistique/bon-de-commande-externe/${data.id}`,
        data.bonCommandeExterne
      );
      thunkAPI.dispatch(
        enqueueSnackbar({
          message: "Bon commande Externe mis à jour avec succès",
          options: {
            variant: "success",
          },
        })
...
export const bonCommandeExterneSlice = createSlice({
  ...
  extraReducers: {
    ...
    [updateBonCommandeExterne.pending.type]: (state) => {
      state.loading = true;
    },
    [updateBonCommandeExterne.fulfilled.type]: (state, action) => {
      state.loading = false;
      state.isEditing = false;
      state.bonCommandeExterne = {};
    },
    [updateBonCommandeExterne.rejected.type]: (state, action) => {
      state.error = action.error;
      state.loading = false;
    },
  },

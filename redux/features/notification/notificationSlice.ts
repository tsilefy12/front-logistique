...
export const notificationSlice = createSlice({
  ...
  reducers: {
    enqueueSnackbar: (state, action: PayloadAction<NotificationItem>) => {
      state.notifications.push({ key: action.payload.key || new Date().getTime() + Math.random(), ...action.payload })
    },
...
export const { enqueueSnackbar, removeSnackbar, closeSnackbar } = notificationSlice.actions;
